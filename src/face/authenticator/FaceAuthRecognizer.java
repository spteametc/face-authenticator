 ...
 *
 *
 * FaceAuthRecognizer is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 2 of the License, or
 * (at your option) any later version (subject to the "Classpath" exception
 * as provided in the LICENSE.txt file that accompanied this code).
 *
 * FaceAuthRecognizer is distributed in the hope that it will be useful,
 ...
 * @author reed
 */
public class FaceAuthRecognizer {

    /**
     * the logger
     */
    private static final Logger LOGGER = Logger.getLogger(FaceAuthRecognizer.class.getName());
    /**
    ...
    /**
     * Constructs a new FaceAuthRecognizer instance.
     */
    public FaceAuthRecognizer() {
    }
     ...
     */
    public String recognizeFileList(final String szFileTest, final String personName) {
        LOGGER.info("===========================================");
        ...
        for (i = 0; i < nTestFaces; i++) {
            ...
            iNearest = findNearestNeighbor(projectedTestFace, new FloatPointer(pConfidence), (personName != null) ? personName : null);
            ...
            if (nearest != 0 && nearest <= personNames.size() && confidence >= 0.70f) {
                name = personNames.get(nearest - 1);
            }
        }
        ...
        if (nCorrect + nWrong > 0) {
            ...
            LOGGER.info("TOTAL TIME: " + (tallyFaceRecognizeTime / (cvGetTickFrequency() * 1000.0 * (nCorrect + nWrong))) + " ms average." + " name:" + name);
        }
        return name;
    }
    ...
    //returns the name of a person based on the index in the training data
    public String getNameByIndex(int i, final String filename) {
        String personName;
        try {
            // read person number (beginning with 1), their name and the image filename.
            final String line = Files.readAllLines(Paths.get(filename)).get(i);
    ...
    //returns the index of the first photo of a person based on name in the training data, -1 if name not found
    public int getIndexByName(final String name) {
        int i = 0;

        try (BufferedReader br = new BufferedReader(new FileReader("./database/input.txt"))) {
            String line;
     ...
     * @return the index
     */
    private int findNearestNeighbor(float projectedTestFace[], FloatPointer pConfidencePointer, final String name) {
...
//          }
            ...
            if(j == -1) {
                LOGGER.info("Failed to find person with name: " + name);
                return 0;
            }
            
            LOGGER.info("find nearest neighbor from training image nr:" + j + " to training image" + (int)(j+(int)20)+ "ntrain: " + nTrainFaces);
            int counter = 0;
            for (iTrain = j; iTrain <= j+20-1; iTrain++) {
                //LOGGER.info("considering training face " + (iTrain + 1));
                double distSq = 0;
                counter++;
...
//          }
            ...
            // and very different images should give a confidence between 0.0 to 0.5.
            // / (float) (nTrainFaces * nEigens))
            float pConfidence = (float) (1.0f - Math.sqrt(leastDistSq / (float) (20 * nEigens)) / 255.0f);
            pConfidencePointer.put(pConfidence);
